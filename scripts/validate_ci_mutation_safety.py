...
AUDIT_BYPASS = re.compile(r"\baudit(?:[:\w-]*)?\b[^#]*(?:\|\||;)\s*(?:true|:)(?:\s|$)", re.IGNORECASE)
...
PAYLOAD_MARKER = re.compile(r"\.payload\.|payload\.(?:b64|tar(?:\.gz)?)|payload[_-]?checksum", re.IGNORECASE)
ACTUAL_ASSIGNMENT = re.compile(r"(?:actual|computed)(?:_payload)?_?checksum[^\n]*sha256sum", re.IGNORECASE)
ACTUAL_OUTPUT = re.compile(r"(?:computed|actual)[^\n]*payload[^\n]*checksum", re.IGNORECASE)
...
def staged_diff(line: str, quiet: bool) -> bool:
    ...
    return bool(GIT_DIFF.search(line)) and ("--cached" in line or "--staged" in line) and (("--quiet" in line) is quiet)
...
def payload_issues(step: StepBlock, path: str) -> List[SafetyIssue]:
    ...
    issues: List[SafetyIssue] = []
    if not (ACTUAL_ASSIGNMENT.search(text) and ACTUAL_OUTPUT.search(text)):
        issues.append(SafetyIssue(path, line, "CIW004", "Payload muss die berechnete SHA-256 ausgeben."))
    if not (EXPECTED_VALUE.search(text) and EXPECTED_OUTPUT.search(text)):
        issues.append(SafetyIssue(path, line, "CIW005", "Payload muss die erwartete SHA-256 ausgeben."))
    if not (FRAGMENT_MARKER.search(text) and FRAGMENT_SIZE.search(text) and FRAGMENT_CHECKSUM.search(text)):
        issues.append(SafetyIssue(path, line, "CIW006", "Payload muss Größe und SHA-256 jedes Fragments ausgeben."))
    if not ARCHIVE_LISTING.search(text):
        issues.append(SafetyIssue(path, line, "CIW007", "Payload-Fehler braucht eine tar -tzf-Diagnose."))
    if not (CHECKSUM_COMPARISON.search(text) and NONZERO_EXIT.search(text)):
        issues.append(SafetyIssue(path, line, "CIW010", "Prüfsummenabweichungen müssen mit Fehlercode enden."))
    return issues
