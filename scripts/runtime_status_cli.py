...
try:  # Support both ``python scripts/...`` and package imports in tests.
    ...
    from .automation_status import WORKFLOWS
...
def build_parser() -> argparse.ArgumentParser:
    ...
    lifecycle.add_argument(
        "--new-run",
        action="store_true",
        help="replace any previous lifecycle",
    )
    lifecycle.add_argument("--finish", choices=("success", "failed"), default=None)
    lifecycle.add_argument(
        "--restore-from",
        type=Path,
        default=None,
        help="restore a validated pre-finalization status atomically",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.restore_from is not None:
        payload = restore_status_file(args.path, args.restore_from)
    elif args.new_run:
        payload = start_run(
            args.path,
            args.workflow or "knowledge-graph",
            phase=args.phase or "initialize",
        ...
        if args.error_class is not None:
            update_kwargs["error_class"] = args.error_class
        if args.error_message is not None:
            update_kwargs["error_message"] = args.error_message
        if args.recovery_action is not None:
            update_kwargs["recovery_action"] = args.recovery_action
        payload = update_status(args.path, **update_kwargs)
