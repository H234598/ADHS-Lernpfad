...
def _paginated_check_runs(repository: str, head_sha: str, token: str) -> list[dict[str, Any]]:
    ...
    while True:
        ...
        batch = [item for item in payload.get("check_runs", []) if isinstance(item, dict)]
        runs.extend(batch)
        if len(batch) < 100:
            return runs
        page += 1
...
def _main_proof(repository: str, merge_sha: str | None, token: str) -> tuple[str, bool]:
    ...
    return main_sha, str(comparison.get("status") or "").casefold() in {"ahead", "identical"}
...
def evaluate_snapshot(
    *,
    ...
    """Read GitHub + canonical status without mutating either source."""

    status = _load_latest(store)
    pull = _mapping(_request(f"{API}/repos/{repository}/pulls/{pr_number}", token))
    ...
    main_sha, main_contains_merge = _main_proof(repository, decision.merge_sha, token)
    ...
    if decision.action == "complete_merged_run" and not main_contains_merge:
        decision = ReconciliationDecision(
            ...
            reasons=(
                *decision.reasons,
                "Der Merge-Commit ist nicht als Bestandteil des aktuellen main nachgewiesen.",
            ),
        )
...
def _decision(payload: Mapping[str, Any]) -> ReconciliationDecision:
    ...
    return ReconciliationDecision(
        ...
        required_checks={
            str(name): {str(key): str(value) for key, value in _mapping(summary).items()}
            for name, summary in _mapping(required).items()
        },
        reasons=tuple(str(item) for item in raw.get("reasons", []) if isinstance(item, str)),
    )


def _branch_exists(repository: str, head_ref: str) -> bool:
    if not head_ref.startswith("agent/einheit-"):
        return False
    result = subprocess.run(  # nosec B603 -- fixed git command and validated branch prefix
        ["git", "ls-remote", "--exit-code", "--heads", "origin", f"refs/heads/{head_ref}"],
        check=False,
...
def _cleanup_branch(head_ref: str) -> bool:
    """Try to delete the already-merged same-repository unit branch.

    Return whether the branch still exists after the attempt.  Failure is
    recorded but does not roll back an already verified merge.
    """

    if not head_ref.startswith("agent/einheit-"):
        return True
    delete = subprocess.run(  # nosec B603 -- fixed git command and validated branch prefix
...
def apply_snapshot(*, snapshot: Mapping[str, Any], store: StatusStore) -> dict[str, Any]:
    decision = _decision(snapshot)
    head_ref = str(snapshot.get("head_ref") or "")
    initial_branch_exists = _branch_exists(str(snapshot.get("repository") or ""), head_ref)
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output", type=Path, default=Path("build/unit-pr-reconciliation.json"))
    ...
    if args.evaluate and (not args.repository or not args.pr_number or not args.token):
