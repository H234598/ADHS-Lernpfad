...
```mermaid
flowchart TD
  A[seit der Entwicklung bestehendes ADHS-Muster] --> G[Ausgangsniveau rekonstruieren]
  B[neue, zeitlich abgegrenzte Veränderung] --> E[episodische Hypothese prüfen]
  S[Stimmung, Schlafbedürfnis, Energie, Aktivität, Sprache und Selbstbild] --> E
  M[Medikamente, Substanzen, Schlafmangel und körperliche Ursachen] --> D[weitere Erklärungen prüfen]
  G --> K[klinische Gesamtformulierung]
  E --> K
  D --> K
  K --> Z[getrennte Behandlungsziele und Verlaufsbeobachtung]
```
