...
while (( $# )); do
  case "$1" in
    ...
    --help|-h)
      cat <<'EOF'
Verwendung: ./Sync/iOS/uninstall-ish.sh [Optionen]
  --purge-config      Konfiguration entfernen
  --remove-checkout   privaten Git-Checkout entfernen
Der in iOS Files eingebundene Obsidian-Vault wird niemals gelöscht.
EOF
      exit 0
      ;;
...
printf 'iSH-Sync entfernt. Der eingebundene Vault blieb unverändert.\n'
