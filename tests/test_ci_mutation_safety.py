...
class MutationSafetyTests(unittest.TestCase):
    ...
    def test_quoted_git_words_do_not_create_writer(self) -> None:
        """Ignore git-looking text that is merely an argument to output commands."""
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            write_workflow(
                root,
...
""",
    ...
    def test_split_git_commit_is_detected(self) -> None:
        """Detect a git mutation whose executable and subcommand span lines."""
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            write_workflow(
                root,
...
""",
    ...
    def test_writer_diagnostics_must_precede_noop_guard(self) -> None:
        """Reject diagnostics placed after the guard they are meant to explain."""
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            write_workflow(
                root,
                """name: Misordered writer
on: workflow_dispatch
jobs:
  write:
    runs-on: ubuntu-24.04
...
"""
    ...
    def test_opaque_payload_checksum_is_rejected(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            codes = {issue.code for issue in validate_repository(root)}
            required = {"CIW004", "CIW005", "CIW006", "CIW007", "CIW010"}
            self.assertTrue(required.issubset(codes))

    def test_commented_payload_evidence_is_rejected(self) -> None:
        """Do not accept checksum evidence that exists only in comments."""
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            start = SAFE_WRITER.index("          expected_payload_checksum")
            end = SAFE_WRITER.index("          npm run audit:dependencies")
            commented = """          payload_file="$RUNNER_TEMP/payload.b64"
          # expected_payload_checksum='abc123'
          # actual_payload_checksum="$(sha256sum "$payload_file" | cut -d' ' -f1)"
          # printf 'Computed payload checksum: %s\\n' "$actual_payload_checksum"
          # printf 'Expected payload checksum: %s\\n' "$expected_payload_checksum"
          # sha256sum "$fragment"
          # tar -tzf "$RUNNER_TEMP/payload.tar.gz"
          # test "$actual_payload_checksum" = "$expected_payload_checksum"
          # exit 1
"""
            write_workflow(
                root,
                SAFE_WRITER[:start] + commented + SAFE_WRITER[end:],
            )
            codes = {issue.code for issue in validate_repository(root)}
            required = {"CIW004", "CIW005", "CIW006", "CIW007", "CIW010"}
            self.assertTrue(required.issubset(codes))
    ...
    def test_printed_checksums_without_enforced_comparison_are_rejected(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            replacement = '          tar -tzf "$RUNNER_TEMP/payload.tar.gz" || true\n'
            write_workflow(root, SAFE_WRITER.replace(comparison, replacement))
            codes = {issue.code for issue in validate_repository(root)}
    ...
    def test_audit_bypasses_are_rejected(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            write_workflow(
                ...
                SAFE_WRITER.replace(
                    "npm run audit:dependencies",
                    "npm run audit:dependencies || exit 0",
                ),
                "exit-zero.yml",
            )
            codes = [issue.code for issue in validate_repository(root)]
