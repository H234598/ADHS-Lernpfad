...
"""
    ...
    def test_opaque_payload_checksum_is_rejected(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            codes = {issue.code for issue in validate_repository(root)}
            self.assertTrue({"CIW004", "CIW005", "CIW006", "CIW007", "CIW010"}.issubset(codes))
    ...
    def test_printed_checksums_without_enforced_comparison_are_rejected(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            write_workflow(root, SAFE_WRITER.replace(comparison, '          tar -tzf "$RUNNER_TEMP/payload.tar.gz" || true\n'))
            codes = {issue.code for issue in validate_repository(root)}
    ...
    def test_audit_bypasses_are_rejected(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            write_workflow(root, SAFE_WRITER.replace("npm run audit:dependencies", "npm run audit:dependencies || true"), "shell.yml")
            write_workflow(root, SAFE_WRITER.replace("      - name: Apply verified payload\n", "      - name: Apply verified payload\n        continue-on-error: true\n"), "step.yml")
            write_workflow(root, SAFE_WRITER.replace("npm run audit:dependencies", "npm run audit:dependencies \\\n            || true"), "multiline.yml")
            codes = [issue.code for issue in validate_repository(root)]
